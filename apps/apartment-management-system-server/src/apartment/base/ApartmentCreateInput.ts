/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { CostCreateNestedManyWithoutApartmentsInput } from "./CostCreateNestedManyWithoutApartmentsInput";
import { ValidateNested, IsOptional, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { PaymentCreateNestedManyWithoutApartmentsInput } from "./PaymentCreateNestedManyWithoutApartmentsInput";

@InputType()
class ApartmentCreateInput {
  @ApiProperty({
    required: false,
    type: () => CostCreateNestedManyWithoutApartmentsInput,
  })
  @ValidateNested()
  @Type(() => CostCreateNestedManyWithoutApartmentsInput)
  @IsOptional()
  @Field(() => CostCreateNestedManyWithoutApartmentsInput, {
    nullable: true,
  })
  costs?: CostCreateNestedManyWithoutApartmentsInput;

  @ApiProperty({
    required: false,
    type: () => PaymentCreateNestedManyWithoutApartmentsInput,
  })
  @ValidateNested()
  @Type(() => PaymentCreateNestedManyWithoutApartmentsInput)
  @IsOptional()
  @Field(() => PaymentCreateNestedManyWithoutApartmentsInput, {
    nullable: true,
  })
  payments?: PaymentCreateNestedManyWithoutApartmentsInput;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsInt()
  @IsOptional()
  @Field(() => Number, {
    nullable: true,
  })
  unitNumber?: number | null;
}

export { ApartmentCreateInput as ApartmentCreateInput };
