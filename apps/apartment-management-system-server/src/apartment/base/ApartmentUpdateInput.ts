/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { CostUpdateManyWithoutApartmentsInput } from "./CostUpdateManyWithoutApartmentsInput";
import { ValidateNested, IsOptional, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { PaymentUpdateManyWithoutApartmentsInput } from "./PaymentUpdateManyWithoutApartmentsInput";

@InputType()
class ApartmentUpdateInput {
  @ApiProperty({
    required: false,
    type: () => CostUpdateManyWithoutApartmentsInput,
  })
  @ValidateNested()
  @Type(() => CostUpdateManyWithoutApartmentsInput)
  @IsOptional()
  @Field(() => CostUpdateManyWithoutApartmentsInput, {
    nullable: true,
  })
  costs?: CostUpdateManyWithoutApartmentsInput;

  @ApiProperty({
    required: false,
    type: () => PaymentUpdateManyWithoutApartmentsInput,
  })
  @ValidateNested()
  @Type(() => PaymentUpdateManyWithoutApartmentsInput)
  @IsOptional()
  @Field(() => PaymentUpdateManyWithoutApartmentsInput, {
    nullable: true,
  })
  payments?: PaymentUpdateManyWithoutApartmentsInput;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsInt()
  @IsOptional()
  @Field(() => Number, {
    nullable: true,
  })
  unitNumber?: number | null;
}

export { ApartmentUpdateInput as ApartmentUpdateInput };
